...
#==================================================
# * [ 注意 ] Skill 节点会自动进行注册，若不想注册，设置 
#    register 属性为 false 即可
#==================================================
# @datetime: 2021-12-12 11:51:53
#==================================================
class_name BaseDurationSkill
extends BaseSkill
...
#	call_deferred("add_child", __duration_timer__)
	add_child(__duration_timer__)
...
func _interrupt() -> int:
	if not __state__.is_no_cast():
		...
		if __state__.is_casting():
			set_interrupt_state(Signals.ExecuteDuration)
			_duration_finished()
			__duration_timer__.stop()
			__state__.casted()
